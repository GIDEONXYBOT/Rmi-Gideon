          ...
          {loading ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm">Loading fight data...</p>
              </div>
            </div>
          ) : error ? (
            <div className="bg-red-900 bg-opacity-30 border border-red-700 rounded p-3">
              ...
              <button
                onClick={() => fetchData(false)}
                className="mt-2 bg-red-700 hover:bg-red-600 text-white px-2 py-1 rounded text-xs"
              >
                Retry
              </button>
            </div>
            ...
            <div className="text-center py-8 text-gray-400">
              <p className="text-sm mb-2">No fight data available yet</p>
              <p className="text-xs text-gray-500">Waiting for first fight to start...</p>
            </div>
          ) : (
                ...
                <div className="text-center text-gray-400">
                  {loading ? (
                    <>
                      ...
                      <div className="text-lg">Loading fight data...</div>
                    </>
                  ) : error ? (
                    <>
                      <div className="text-2xl mb-4">⚠️ Error Loading Data</div>
                      <div className="text-sm text-red-400 mb-4 break-words">{error}</div>
                      <button
                        onClick={() => fetchData(false)}
                    ...
                    <>
                      ...
                      <div className="text-lg mb-4">Waiting for next fight to begin...</div>
                      <div className="text-xs text-gray-500">
                        <p>Draws loaded: {draws.length}</p>
                        <p>API Sources: {Object.entries(apiSources).filter(([_, enabled]) => enabled).map(([source]) => source).join(', ')}</p>
                      </div>
                    </>
                  )}
                </div>
